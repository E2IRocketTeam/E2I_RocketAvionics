...
bool BNO085::begin() {
    ...
    // 회전 벡터와 '일반 가속도' 리포트를 활성화합니다.
    if (!bno.enableReport(SH2_ROTATION_VECTOR, 5000)) { // 5ms (200Hz)
        return false;
    }
    // SH2_ACCELEROMETER를 사용하여 중력가속도를 포함한 원시 가속도 데이터를 요청합니다.
    if (!bno.enableReport(SH2_ACCELEROMETER, 5000)) { // 5ms (200Hz)
        return false;
    }
    ...
    if (bno.getSensorEvent(&sensorValue)) {
        switch (sensorValue.sensorId) {
            ...
            case SH2_ACCELEROMETER:
                // 중력가속도가 포함된 가속도 값을 accelerometer 구조체에 저장합니다.
